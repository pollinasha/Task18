...
class Practicum {
	...
	private static void addUser() {
		...
		} catch (ValidateNameException validateNameException) {
			System.out.println("Ошибка валидации имени: " + validateNameException.getMessage());
		} catch (ValidatePasswordException validatePasswordException) {
			System.out.println("Ошибка валидации пароля: " + validatePasswordException.getMessage());
	...
	private static void showUserPassword() {
		...
		try {
			...
			System.out.printf("\nПароль пользователя %s = %s", name, password);
		} catch (ValidateNameException validateNameException) {
			System.out.println("Ошибка валидации имени: " + validateNameException.getMessage());
		} catch (ValidatePasswordException validatePasswordException) {
			System.out.println("Ошибка валидации пароля: " + validatePasswordException.getMessage());
